...
const ViewCritiqueUpdated: React.FC = () => {
  ...
  const reviewPermissions = useReviewPermissions(
    critiqueId || '',
    critique?.adjudicatorId || ''
  );
    ...
    const loadCritique = async () => {
      ...
      try {
        ...
        const video = Array.isArray(mockVideoSubmissions)
          ? mockVideoSubmissions.find(v => v && v.id === foundCritique.submissionId)
          : null;
        ...
        const critiqueData: CritiqueData = {
          ...
          adjudicatorId: 'adj-123', // Mock adjudicator ID
        ...
        const status = await paymentService.getCritiquePaymentStatus(critiqueId);
        if (status) {
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center">
                  <Play className="h-5 w-5 mr-2" />Video Critique
                </CardTitle>
              </CardHeader>
              <CardContent>
                <PlaybackPreviewPlayer videoId={critique.videoId} critiqueId={critique.id} />
              </CardContent>
            </Card>
            
            <PostCritiqueAI critiqueId={critique.id} audioUrl={critique.audioUrl} />
            
            {/* Review Form - Only show to clients who haven't reviewed */}
            {reviewPermissions.userRole === 'client' && 
             reviewPermissions.canReview && 
             !reviewPermissions.loading && (
              <AdjudicatorReviewForm
                critiqueId={critique.id}
                adjudicatorId={critique.adjudicatorId}
                adjudicatorName={critique.adjudicatorName}
                onReviewSubmitted={handleReviewSubmitted}
              />
            )}
          </div>
          <div className="space-y-6">
            <CritiqueFeedbackPanel critiqueId={critique.id} audioUrl={critique.audioUrl} />
          </div>
        </div>
      </div>
    </AppLayout>
...
export default ViewCritiqueUpdated;