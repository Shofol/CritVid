...
export async function uploadVideo(file: File, path?: string): Promise<string> {
  try {
    ...
    try {
      ...
      console.warn('Error uploading video to Supabase, using memory cache:', error);
 ...
 * @returns The file path in the bucket
 */
export async function uploadCritique(file: File | Blob, fileName: string, path?: string): Promise<string> {
  try {
    ...
    try {
      ...
      console.warn('Error uploading critique to Supabase, using memory cache:', supabaseError);
      return filePath; // Return the path even though it's only in memory cache
    }
  } catch (error) {
...
export async function downloadFile(bucket: string, path: string): Promise<Blob> {
  try {
    ...
    console.warn('Error downloading from Supabase, checking memory cache:', error);
...
export async function deleteFile(bucket: string, path: string): Promise<void> {
  try {
    ...
    try {
      ...
      console.warn('Error deleting file from Supabase (but removed from memory cache):', supabaseError);
    }
  } catch (error) {
