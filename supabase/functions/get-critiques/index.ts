...
serve(async (req) => {
  ...
  try {
    ...
    // Build the base query with correct foreign key constraint names
    let query = supabaseClient.from("critiques").select(`
        *,
        user:users (
          id,
          email,
    ...
    console.log("Raw critiques data:", critiques);

    // Transform the data to match the expected format
    const transformedCritiques =
    ...
    return new Response(
      JSON.stringify({
        data: transformedCritiques,
        count: transformedCritiques.length,
        debug: {
          rawData: critiques,
          relationships: critiques?.map((c) => ({
            id: c.id,
            hasUser: !!c.user,
            hasAdjudicator: !!c.adjudicator,
            hasVideo: !!c.video,
            hasReview: !!c.review,
          })),
        },
